...
pub fn simulate_validators(
    ...
    validators
        ...
        .map(|(validator_id, secret_key)| {
            ...
            let decision = if report.accepted { "accept" } else { "reject" };
            ...
            let mut attestation =
                ValidatorAttestation::new(validator_id, decision, digest.clone(), timestamp);
...
impl AgentGate {
    ...
    pub fn run(&self, context: &str, kwargs: &HashMap<String, Value>) -> Result<AgentResult, String> {
        ...
        let proof = chain.seal(Some(metadata))?;
        Ok(AgentResult {
...
impl OrchestratingAgent {
    ...
    pub fn emit(
        ...
        let result = match event {
            ...
            PipelineEvent::ProdErrorSpike => Some(self.handle_prod_error_spike(context, &kwargs)),
            _ => None,
        };
        self.record_audit(&event, result.clone().unwrap_or_else(|| "no_handler".to_string()), kwargs);
        result
    }
